...
Use our lightweight command-line interface to inspect and download NASNet:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info NASNet

# Print performance and accuracy metrics
qai-hub-models perf NASNet
qai-hub-models numerics NASNet

# Download a ready-to-deploy asset
qai-hub-models fetch NASNet --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.nasnet.demo { --quantize w8a8_mixed_fp16 }
```
...
```bash
qai-hub-models export nasnet --target-runtime tflite --precision float
```
