...
Use our lightweight command-line interface to inspect and download HRNetFace:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info HRNetFace

# Print performance and accuracy metrics
qai-hub-models perf HRNetFace
qai-hub-models numerics HRNetFace

# Download a ready-to-deploy asset
qai-hub-models fetch HRNetFace --runtime tflite --precision float
```
...
```bash
qai-hub-models export hrnet_face --target-runtime tflite --precision float
```
