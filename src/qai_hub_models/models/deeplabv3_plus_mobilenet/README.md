...
Use our lightweight command-line interface to inspect and download DeepLabV3-Plus-MobileNet:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info DeepLabV3-Plus-MobileNet

# Print performance and accuracy metrics
qai-hub-models perf DeepLabV3-Plus-MobileNet
qai-hub-models numerics DeepLabV3-Plus-MobileNet

# Download a ready-to-deploy asset
qai-hub-models fetch DeepLabV3-Plus-MobileNet --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.deeplabv3_plus_mobilenet.demo { --quantize w8a16, w8a8 }
```
...
```bash
qai-hub-models export deeplabv3_plus_mobilenet --target-runtime tflite --precision float
```
