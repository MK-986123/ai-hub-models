...
This is based on the implementation of Bert-Base-Uncased-Hf found [here](https://github.com/google-research/bert).
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/bert_base_uncased_hf).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download Bert-Base-Uncased-Hf:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Bert-Base-Uncased-Hf

# Print performance and accuracy metrics
qai-hub-models perf Bert-Base-Uncased-Hf
qai-hub-models numerics Bert-Base-Uncased-Hf

# Download a ready-to-deploy asset
qai-hub-models fetch Bert-Base-Uncased-Hf --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.bert_base_uncased_hf.demo { --quantize w8a16 }
```
...
```bash
qai-hub-models export bert_base_uncased_hf --target-runtime tflite --precision float
```
