# [YOLOE-Segmentation: Real-time object segmentation optimized for mobile and edge by Ultralytics](https://aihub.qualcomm.com/models/yoloe_seg)
...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/yoloe_seg).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect YOLOE-Segmentation:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model's metadata
qai-hub-models info YOLOE-Segmentation

# Print performance and accuracy metrics
qai-hub-models perf YOLOE-Segmentation
qai-hub-models numerics YOLOE-Segmentation
...
pip install "qai-hub-models[yoloe-seg]"
```
...
```bash
python -m qai_hub_models.models.yoloe_seg.demo { --quantize w8a8, w8a16 }
```
...
```bash
qai-hub-models export yoloe_seg --target-runtime tflite --precision float
```
