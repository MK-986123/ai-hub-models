...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/distil_bert_base_uncased_hf).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download Distil-Bert-Base-Uncased-Hf:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Distil-Bert-Base-Uncased-Hf

# Print performance and accuracy metrics
qai-hub-models perf Distil-Bert-Base-Uncased-Hf
qai-hub-models numerics Distil-Bert-Base-Uncased-Hf

# Download a ready-to-deploy asset
qai-hub-models fetch Distil-Bert-Base-Uncased-Hf --runtime tflite --precision float
```
...
```bash
qai-hub-models export distil_bert_base_uncased_hf --target-runtime tflite --precision float
```
