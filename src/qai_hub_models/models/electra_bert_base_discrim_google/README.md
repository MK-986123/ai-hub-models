...
Use our lightweight command-line interface to inspect and download Electra-Bert-Base-Discrim-Google:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Electra-Bert-Base-Discrim-Google

# Print performance and accuracy metrics
qai-hub-models perf Electra-Bert-Base-Discrim-Google
...
```bash
qai-hub-models export electra_bert_base_discrim_google --target-runtime tflite --precision float
```
