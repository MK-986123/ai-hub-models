...
This is based on the implementation of Segment-Anything-Model found [here](https://github.com/facebookresearch/segment-anything).
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/sam).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download Segment-Anything-Model:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Segment-Anything-Model

# Print performance and accuracy metrics
qai-hub-models perf Segment-Anything-Model
qai-hub-models numerics Segment-Anything-Model

# Download a ready-to-deploy asset
qai-hub-models fetch Segment-Anything-Model --runtime qnn_context_binary --precision float
```
...
```bash
qai-hub-models export sam --target-runtime qnn_context_binary --precision float
```
