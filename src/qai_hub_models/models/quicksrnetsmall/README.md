...
Use our lightweight command-line interface to inspect and download QuickSRNetSmall:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info QuickSRNetSmall

# Print performance and accuracy metrics
qai-hub-models perf QuickSRNetSmall
qai-hub-models numerics QuickSRNetSmall

# Download a ready-to-deploy asset
qai-hub-models fetch QuickSRNetSmall --runtime tflite --precision float
```
...
```bash
qai-hub-models export quicksrnetsmall --target-runtime tflite --precision float
```
