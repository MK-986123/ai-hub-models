...
Use our lightweight command-line interface to inspect and download BEVFusion:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info BEVFusion

# Print performance and accuracy metrics
qai-hub-models perf BEVFusion
qai-hub-models numerics BEVFusion

# Download a ready-to-deploy asset
qai-hub-models fetch BEVFusion --runtime qnn_context_binary --precision float
```
...
```bash
qai-hub-models export bevfusion_det --target-runtime qnn_context_binary --precision float --device "Samsung Galaxy S25 (Family)"
```
