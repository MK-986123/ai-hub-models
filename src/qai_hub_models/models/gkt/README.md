...
Use our lightweight command-line interface to inspect and download GKT:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info GKT

# Print performance and accuracy metrics
qai-hub-models perf GKT
qai-hub-models numerics GKT

# Download a ready-to-deploy asset
qai-hub-models fetch GKT --runtime qnn_context_binary --precision float
```
...
```bash
qai-hub-models export gkt --target-runtime qnn_context_binary --precision float --device "Samsung Galaxy S25 (Family)"
```
