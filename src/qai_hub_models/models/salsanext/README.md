...
Use our lightweight command-line interface to inspect and download SalsaNext:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info SalsaNext

# Print performance and accuracy metrics
qai-hub-models perf SalsaNext
qai-hub-models numerics SalsaNext

# Download a ready-to-deploy asset
qai-hub-models fetch SalsaNext --runtime tflite --precision float
```
...
```bash
qai-hub-models export salsanext --target-runtime tflite --precision float
```
