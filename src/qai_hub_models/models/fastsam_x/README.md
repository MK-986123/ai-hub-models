...
Use our lightweight command-line interface to inspect and download FastSam-X:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info FastSam-X

# Print performance and accuracy metrics
qai-hub-models perf FastSam-X
qai-hub-models numerics FastSam-X

# Download a ready-to-deploy asset
qai-hub-models fetch FastSam-X --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.fastsam_x.demo { --quantize w8a16 }
```
...
```bash
qai-hub-models export fastsam_x --target-runtime tflite --precision float
```
