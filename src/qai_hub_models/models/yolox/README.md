...
Use our lightweight command-line interface to inspect and download Yolo-X:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Yolo-X

# Print performance and accuracy metrics
qai-hub-models perf Yolo-X
...
```bash
python -m qai_hub_models.models.yolox.demo { --quantize w8a8_mixed_int16, w8a16, w8a8 }
```
...
```bash
qai-hub-models export yolox --target-runtime onnx --precision float
```
