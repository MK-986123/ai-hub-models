...
Use our lightweight command-line interface to inspect and download HRNet-W48-OCR:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info HRNet-W48-OCR

# Print performance and accuracy metrics
qai-hub-models perf HRNet-W48-OCR
qai-hub-models numerics HRNet-W48-OCR

# Download a ready-to-deploy asset
qai-hub-models fetch HRNet-W48-OCR --runtime onnx --precision float
```
...
```bash
qai-hub-models export hrnet_w48_ocr --target-runtime onnx --precision float
```
