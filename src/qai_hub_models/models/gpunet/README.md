...
Use our lightweight command-line interface to inspect and download GPUNet:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info GPUNet

# Print performance and accuracy metrics
qai-hub-models perf GPUNet
qai-hub-models numerics GPUNet

# Download a ready-to-deploy asset
qai-hub-models fetch GPUNet --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.gpunet.demo { --quantize w8a16, w8a8 }
```
...
```bash
qai-hub-models export gpunet --target-runtime tflite --precision float
```
