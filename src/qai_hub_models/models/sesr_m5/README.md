...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/sesr_m5).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download SESR-M5:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info SESR-M5

# Print performance and accuracy metrics
qai-hub-models perf SESR-M5
qai-hub-models numerics SESR-M5

# Download a ready-to-deploy asset
qai-hub-models fetch SESR-M5 --runtime tflite --precision float
```
...
```bash
qai-hub-models export sesr_m5 --target-runtime tflite --precision float
```
