...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/yolov10_det).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect YOLOv10-Detection:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model's metadata
qai-hub-models info YOLOv10-Detection

# Print performance and accuracy metrics
qai-hub-models perf YOLOv10-Detection
qai-hub-models numerics YOLOv10-Detection
...
```bash
python -m qai_hub_models.models.yolov10_det.demo { --quantize w8a8_mixed_int16, w8a16, w8a8 }
```
...
```bash
qai-hub-models export yolov10_det --target-runtime tflite --precision float
```
