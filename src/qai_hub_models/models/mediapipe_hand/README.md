...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/mediapipe_hand).
...
qai-hub-models fetch MediaPipe-Hand-Detection --runtime tflite --precision float
```
...
```bash
qai-hub-models export mediapipe_hand --target-runtime tflite --precision float
```
