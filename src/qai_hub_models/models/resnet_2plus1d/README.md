...
Use our lightweight command-line interface to inspect and download ResNet-2Plus1D:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info ResNet-2Plus1D

# Print performance and accuracy metrics
qai-hub-models perf ResNet-2Plus1D
qai-hub-models numerics ResNet-2Plus1D

# Download a ready-to-deploy asset
qai-hub-models fetch ResNet-2Plus1D --runtime tflite --precision float
```
...
```bash
qai-hub-models export resnet_2plus1d --target-runtime tflite --precision float
```
