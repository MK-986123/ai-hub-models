...
Use our lightweight command-line interface to inspect and download Mask2Former:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Mask2Former

# Print performance and accuracy metrics
qai-hub-models perf Mask2Former
qai-hub-models numerics Mask2Former

# Download a ready-to-deploy asset
qai-hub-models fetch Mask2Former --runtime qnn_context_binary --precision float
```
...
```bash
qai-hub-models export mask2former --target-runtime qnn_context_binary --precision float --device "Samsung Galaxy S25 (Family)"
```
