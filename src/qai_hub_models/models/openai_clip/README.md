...
Use our lightweight command-line interface to inspect and download OpenAI-Clip:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info OpenAI-Clip

# Print performance and accuracy metrics
qai-hub-models perf OpenAI-Clip
qai-hub-models numerics OpenAI-Clip

# Download a ready-to-deploy asset
qai-hub-models fetch OpenAI-Clip --runtime tflite --precision float
```
...
```bash
qai-hub-models export openai_clip --target-runtime tflite --precision float
```
