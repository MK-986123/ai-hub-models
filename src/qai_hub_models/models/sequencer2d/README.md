...
This is based on the implementation of Sequencer2D found [here](https://github.com/okojoalg/sequencer).
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/sequencer2d).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download Sequencer2D:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Sequencer2D

# Print performance and accuracy metrics
qai-hub-models perf Sequencer2D
qai-hub-models numerics Sequencer2D

# Download a ready-to-deploy asset
qai-hub-models fetch Sequencer2D --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.sequencer2d.demo { --quantize w8a16, w8a8 }
```
...
```bash
qai-hub-models export sequencer2d --target-runtime tflite --precision float
```
