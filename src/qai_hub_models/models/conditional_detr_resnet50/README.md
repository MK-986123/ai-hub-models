...
This repository contains scripts for optimized on-device export suitable to run on Qualcomm® devices. More details on model performance across various devices, can be found [here](https://aihub.qualcomm.com/models/conditional_detr_resnet50).

Qualcomm AI Hub Models uses [Qualcomm AI Hub Workbench](https://workbench.aihub.qualcomm.com) to compile, profile, and evaluate this model. [Sign up](https://myaccount.qualcomm.com/signup) to run these models on a hosted Qualcomm® device.

## Quick Start

Use our lightweight command-line interface to inspect and download Conditional-DETR-ResNet50:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info Conditional-DETR-ResNet50

# Print performance and accuracy metrics
qai-hub-models perf Conditional-DETR-ResNet50
qai-hub-models numerics Conditional-DETR-ResNet50

# Download a ready-to-deploy asset
qai-hub-models fetch Conditional-DETR-ResNet50 --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.conditional_detr_resnet50.demo { --quantize w8a16_mixed_fp16 }
```
...
```bash
qai-hub-models export conditional_detr_resnet50 --target-runtime tflite --precision float
```
