...
Use our lightweight command-line interface to inspect and download EfficientViT-b2-cls:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info EfficientViT-b2-cls

# Print performance and accuracy metrics
qai-hub-models perf EfficientViT-b2-cls
qai-hub-models numerics EfficientViT-b2-cls

# Download a ready-to-deploy asset
qai-hub-models fetch EfficientViT-b2-cls --runtime tflite --precision float
```
...
```bash
qai-hub-models export efficientvit_b2_cls --target-runtime tflite --precision float
```
