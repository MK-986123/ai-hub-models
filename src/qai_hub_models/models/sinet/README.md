...
Use our lightweight command-line interface to inspect and download SINet:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info SINet

# Print performance and accuracy metrics
qai-hub-models perf SINet
qai-hub-models numerics SINet

# Download a ready-to-deploy asset
qai-hub-models fetch SINet --runtime tflite --precision float
```
...
```bash
python -m qai_hub_models.models.sinet.demo { --quantize w8a16, w8a8 }
```
...
```bash
qai-hub-models export sinet --target-runtime tflite --precision float
```
