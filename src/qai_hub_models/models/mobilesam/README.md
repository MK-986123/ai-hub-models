...
Use our lightweight command-line interface to inspect and download MobileSam:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info MobileSam

# Print performance and accuracy metrics
qai-hub-models perf MobileSam
qai-hub-models numerics MobileSam

# Download a ready-to-deploy asset
qai-hub-models fetch MobileSam --runtime tflite --precision float
```
...
```bash
qai-hub-models export mobilesam --target-runtime tflite --precision float
```
