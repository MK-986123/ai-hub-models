...
Use our lightweight command-line interface to inspect and download MediaPipe-Pose-Estimation:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info MediaPipe-Pose-Estimation

# Print performance and accuracy metrics
qai-hub-models perf MediaPipe-Pose-Estimation
qai-hub-models numerics MediaPipe-Pose-Estimation

# Download a ready-to-deploy asset
qai-hub-models fetch MediaPipe-Pose-Estimation --runtime qnn_context_binary --precision float
```
...
```bash
qai-hub-models export mediapipe_pose --target-runtime qnn_context_binary --precision float --device "Samsung Galaxy S25 (Family)"
```
