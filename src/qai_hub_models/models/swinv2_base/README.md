...
Use our lightweight command-line interface to inspect and download SwinV2-Base:

```bash
pip install qai_hub_models_cli # (the CLI is also available with the qai-hub-models package)

# Inspect the model and list the available download options
qai-hub-models info SwinV2-Base

# Print performance and accuracy metrics
qai-hub-models perf SwinV2-Base
qai-hub-models numerics SwinV2-Base

# Download a ready-to-deploy asset
qai-hub-models fetch SwinV2-Base --runtime tflite --precision float
```
...
```bash
qai-hub-models export swinv2_base --target-runtime tflite --precision float
```
